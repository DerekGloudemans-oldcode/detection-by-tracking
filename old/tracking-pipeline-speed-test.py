...
def plot(im,detections,post_locations,all_classes,class_dict):
    im = im.copy()/255.0
...
if __name__ == "__main__":
        ...
        for det_step in range(1,30,2):
            ...
            for (frame,dim,original_im) in frames:
                ...
                start = time.time()
                if PLOT:
                    plot(original_im,detections,post_locations,all_classes,class_dict)
            ...
            print("\n\nTotal Framerate with step size {}: {:.2f} fps".format(det_step,(n_frames/total_time)))
            print("---------- per operation ----------")
            for key in time_metrics:
                print("{:.3f}s ({:.2f}%) on {}".format(time_metrics[key],time_metrics[key]/total_time*100,key))
                
            
            all_times['x'].append(det_step)
            all_times['total'].append(total_time)
            all_times['detect'].append(time_metrics['detect'])
            all_times['localize'].append(time_metrics['localize'])
            all_times['gpu_load'].append(time_metrics['gpu_load'])
            all_times['match'].append(time_metrics['match'])
            all_times['track'].append(time_metrics['predict']+time_metrics['update']+ time_metrics['add and remove'])
            all_times['fps'].append(n_frames/total_time)
            
            
            
            
        with open("results_960.cpkl","wb") as f:
            pickle.dump(all_times,f)
            
        # plot         
        colors = np.array([[0.3,0.4,0.8],
                           [0.4,0.5,0.7],
                           [0.5,0.4,0.6],
                           [0.6,0.5,0.5],
                           [0.7,0.4,0.4],
                           [0.8,0.5,0.3]])
        alpha = 0.7
        alphas = np.ones([len(colors),1])*alpha
        ...
        for key in all_times:
            if key not in ['x','total','fps']:
                y.append(np.array(all_times[key]))
                legend.append(key)
            if key == 'x':
                x = np.array(all_times[key])
        
        y = np.array(y)
        total = np.array(all_times['total'])
        #y = y/total
        other = (total - np.sum(y,axis = 0))
        y = np.concatenate((y,other[np.newaxis,:]),axis = 0)
        y = y/total
        legend.append("other")
        
        plt.stackplot(x,y, colors = colors)
        plt.legend(legend,fontsize = 18)
        plt.xlabel("Detection skip frames",fontsize = 24)
        plt.ylabel("Relative time usage", fontsize = 24)
        plt.xlim([0,35])
        plt.title("Relative Time Usage for 960x540 Video Tracking",fontsize = 36)
        
        with open("results_4k.cpkl",'rb') as f:
            all_times4k  = pickle.load(f)
        
        fps_4k = all_times4k['fps']
        fps = all_times['fps']
        
        plt.figure()
        plt.plot(x,fps,color = colors[1])
        plt.plot(x,fps_4k,color = colors[4])
        plt.legend(["960x540","4k"],loc = 'lower right',fontsize = 18)
        plt.xlabel("Detection skip frames",fontsize = 24)
        plt.ylabel("Frame Rate (fps)",fontsize = 24)
        plt.title("Frame Rate versus Skip Frames",fontsize = 36)
